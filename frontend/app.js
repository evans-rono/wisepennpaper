...
// server, so there is nothing to fetch on load. This is the same data again as
// JSON, for the parts that stay interactive: the service detail dialog and the
// portfolio filter.
...
function showService(d, s) {
  ...
  d.querySelector('#serviceDetail').innerHTML = `
    ...
    <p class="page-actions"><a class="btn" href="#quote" id="requestService">Request this service</a>
      <a class="btn ghost" href="/services/${esc(s.slug)}">Read the full page</a></p>`;
  d.querySelector('#requestService').addEventListener('click', () => {
    d.close();
...
// The cards are already in the document; filtering hides them rather than
// re-rendering, so the markup lives in exactly one place (the server).
