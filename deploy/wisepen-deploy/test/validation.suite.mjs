...
// The token must be fetched *before* the request is built: supertest's agent
// copies its cookie jar onto the request when you call a.post(), so awaiting
// the token inside the chain leaves that first request with no CSRF cookie.
const post = async (a, path, body) => { const t = await token(a); return a.post(path).set('CSRF-Token', t).send(body); };
const patch = async (a, path, body) => { const t = await token(a); return a.patch(path).set('CSRF-Token', t).send(body); };
